...
class ProductoController {
    listarProductos(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            try {
                yield mysqlConnection.query("select * from producto where estado = 'a' and id_hotel = ? order by nombre_producto asc", [id], (error, result) => {
                    if (error)
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield mysqlConnection.beginTransaction((error) => {
                    ...
                    mysqlConnection.query(`select * from producto where id_producto = ?;`, [req.body.id_producto], (error, result) => {
                        if (error) {
                        ...
                        if (result[0].stock !== req.body.stock) {
                            mysqlConnection.query(`insert into ingreso_producto (
                                cantidad, 
                                id_usuario_op, 
                                id_hotel, 
                                id_producto)
                                values (?, ?, ?, ?);`, [req.body.stock - result[0].stock,
                                req.body.id_usuario_op,
                                req.body.id_hotel,
                                req.body.id_producto], (error, result) => {
                                if (error) {
                                    return mysqlConnection.rollback(() => {
                                        throw error;
                                    });
                        ...
                        mysqlConnection.query(`update producto set ? where id_producto = ?`, [req.body, id], (error, result) => {
                            if (error) {
                                return mysqlConnection.rollback(() => {
                                    throw error;
                                });
